...
public class Logger {
    ...
    // log记录方式
    private static final int  LOG_NULL       = 0; // null device
    private static final int   LOG_CONSOLE   = 1; // console log
    ...
    public static void d(String msg) {
        if (logDevice > LOG_NULL) {
            Logger.showLog(DEBUG, getTag(null, STACK_TRACE_DEEP), " " + msg, logDevice);
        }
    }
        ...
        for (String key : keys) {
            if (b.get(key) instanceof byte[]) {
                Logger.showLog(DEBUG, tag,
                        key + ":" + HexUtil.bytes2HexStr(b.getByteArray(key)), logDevice);
            } else if (b.get(key) instanceof String) {
                Logger.showLog(DEBUG, tag, key + ":" + b.getString(key), logDevice);
            } else if (b.get(key) instanceof Long) {
                Logger.showLog(DEBUG, tag, key + ":" + b.getLong(key), logDevice);
